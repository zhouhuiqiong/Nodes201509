(function () {
    ...
    x.ajax = function (options) {
        ...
        var _error = __options.error;
        __options.error = function (err) {
            _error(err);
            deferred.reject(err);
            deferred.all(err);
        };
        var __success = __options.success;
        __options.success = function (data) {
            __success(data);
    ...
    var tool = {
        ...
        init: function () {
            this.each(['String', 'Array', 'Object', 'Function'], function (item) {
    ...
    tool.each(['get', 'post'], function (item) {
         ...
         * @param data {Object|string} 往服务器发送的数据
         * @param callback {Function} 只是成功时的回调函数
